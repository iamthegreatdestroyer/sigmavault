...
try:
    from ..security.hardening import (
        MemoryBoundedBuffer, StreamingProcessor, 
        validate_file_size, validate_bytes_length
    )
    HAS_HARDENING = True
except ImportError:
    HAS_HARDENING = False
    ...
    class MemoryBoundedBuffer:
        def __init__(self, max_size=None): pass
        def can_allocate(self, size): return True
        def allocate_chunk(self, size): return bytearray(size)
    class StreamingProcessor:
        def __init__(self, chunk_size=1024*1024): self.chunk_size = chunk_size
    def validate_file_size(size, max_size=10*1024*1024*1024): pass
    def validate_bytes_length(data, min_len, max_len, name="data"): pass

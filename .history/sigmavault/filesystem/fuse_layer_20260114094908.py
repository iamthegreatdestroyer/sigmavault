...
# Import security hardening utilities
try:
    from ..security.hardening import RWLock, synchronized_method, ManagedResource
    HAS_HARDENING = True
except ImportError:
    HAS_HARDENING = False
    # Fallback: use standard RLock
    class RWLock:
        def __init__(self):
            self._lock = threading.RLock()
        def acquire_read(self): self._lock.acquire()
        def release_read(self): self._lock.release()
        def acquire_write(self): self._lock.acquire()
        def release_write(self): self._lock.release()
    ...
    class ManagedResource:
        ...
        def __exit__(self, *args): self.release(self.acquire())
...
class TransactionState:
    """State of a filesystem transaction."""
    ...
    def add_backup(self, key: str, data: Any):
        """Add backup data for rollback."""
        self.backups[key] = data
    
    def get_backup(self, key: str) -> Any:
        """Get backup data."""
...
class TransactionManager:
    """
    Manages filesystem transactions with rollback capabilities.
    
    Provides atomic operations for file system changes with
    ...
    def begin_transaction(self, tx_id: str) -> TransactionState:
    ...
    def _execute_rollback(self, operation: Dict[str, Any], tx: TransactionState):
        ...
        if op_type == 'store_file':
            ...
            # Remove created file from index
            path = operation['path']
            try:
            ...
            try:
                ...
                print(f"Warning: Failed to remove from cache during rollback: {e}")
                
        elif op_type == 'remove_cache':
            # Restore cache content
            path = operation['path']
            backup_content = tx.get_backup(f'cache_content_{path}')
            if backup_content is not None:
                try:
                    self.cache.put(path, backup_content)
                except Exception as e:
                    print(f"Warning: Failed to restore cache content during rollback: {e}")
                    
        elif op_type == 'delete_index':
            # Restore index entry
            path = operation['path']
            backup_entry = tx.get_backup(f'index_entry_{path}')
            if backup_entry:
                try:
                    # This would need to be implemented in VirtualMetadataIndex
                    # For now, we'll skip this as it's complex
                    pass
                except Exception as e:
                    print(f"Warning: Failed to restore index entry during rollback: {e}")
                    
        elif op_type == 'update_entry':
            ...
            try:
                entry = self.index.get(path)
                if entry:
                    entry.scattered_ref = old_ref
            except Exception as e:
                print(f"Warning: Failed to restore entry reference during rollback: {e}")
                
        elif op_type == 'mark_clean':
            # Mark cache as dirty again
            path = operation['path']
            try:
                self.cache.mark_dirty(path)
            except Exception as e:
                print(f"Warning: Failed to mark cache dirty during rollback: {e}")
            ...
            try:
                ...
                print(f"Warning: Failed to remove file handle during rollback: {e}")
    ...
    def cleanup_expired_transactions(self, max_age: int = 300):
        """Clean up expired transactions (default 5 minutes)."""
        with self._lock:
            ...
            for tx_id, tx in self.active_transactions.items():
                ...
                if tx.operations and (current_time - tx.operations[0]['timestamp']) > max_age:
                    expired.append(tx_id)
            
            for tx_id in expired:
                print(f"Rolling back expired transaction: {tx_id}")
...
class ScatterStorageBackend:
    ...
    def __init__(self, storage_path: Path, scatter_engine: DimensionalScatterEngine, tx_manager: TransactionManager):
        ...
        self.backup_path = storage_path / 'backups'
        self.data_path.mkdir(parents=True, exist_ok=True)
        self.meta_path.mkdir(parents=True, exist_ok=True)
        self.backup_path.mkdir(parents=True, exist_ok=True)
    
    def store(self, file_id: bytes, content: bytes, tx_id: Optional[str] = None) -> str:
        """
        ...
        with self._lock:
            try:
                ...
                # Create backup if in transaction
                if tx:
                    # Backup existing data if any
                    existing = self._load_scattered(ref_id)
                    if existing:
                        tx.add_backup(f'scatter_backup_{ref_id}', existing)
                    tx.add_operation('store_file', ref_id=ref_id, file_id=file_id.hex())
    ...
    def delete(self, ref_id: str, tx_id: Optional[str] = None):
        """Delete scattered file. Thread-safe with transaction support."""
        with self._lock:
            try:
                # Get transaction if provided
                tx = None
                ...
                if tx:
                    ...
                    if existing:
                        tx.add_backup(f'scatter_backup_{ref_id}', existing)
                        tx.add_operation('delete_file', ref_id=ref_id)
...
class SigmaVaultFS(Operations):
    ...
    def __init__(self, storage_path: Path, key_state: KeyState, 
        ...
        # Initialize ML access logger for pattern analysis
        self.vault_id = hashlib.sha256(str(storage_path).encode()).hexdigest()[:16]
        ...
        # Start transaction cleanup thread
        self._cleanup_thread = threading.Thread(
            target=self._transaction_cleanup_worker, 
            daemon=True
        )
    ...
    def _transaction_cleanup_worker(self):
        """Background worker to clean up expired transactions."""
        while True:
            try:
                # Clean up expired transactions every 60 seconds
                time.sleep(60)
                self.tx_manager.cleanup_expired_transactions()
                self.storage.tx_manager.cleanup_expired_transactions()
            except Exception as e:
                print(f"Warning: Transaction cleanup failed: {e}")
    ...
    def _log_access(
        ...
        """
        Log file access event for ML pattern analysis.
    ...
    def create(self, path, mode, fi=None):
        """Create and open file. Thread-safe with transaction support."""
        with self._lock:
            path = self._get_full_path(path)
            
            # Begin transaction
            tx_id = f"create_{path}_{time.time()}"
            tx = self.tx_manager.begin_transaction(tx_id)
            
            try:
                # Create index entry
                entry = self.index.create_file(path, mode)
                tx.add_operation('create_file', path=path, mode=mode)
                
                # Initialize empty content in cache
                self.cache.put(path, b'', dirty=True)
                tx.add_operation('cache_put', path=path, content=b'')
                ...
                tx.add_operation('open_file', fh=fh, path=path)
    ...
    def open(self, path, flags):
        ...
        with self._lock:
            ...
            # Log successful open
            self._log_access(path, "open", entry.size, start_time, success, error_code)
            
            return fh
    
    def read(self, path, size, offset, fh):
        """Read from file. Thread-safe with ML access logging."""
        start_time = time.time()
        success = True
        error_code = None
        bytes_read = 0
        ...
        with self._lock:
            ...
            if content is None:
                success = False
                error_code = "ENOENT"
                self._log_access(path, "read", 0, start_time, success, error_code)
                raise FuseOSError(errno.ENOENT)
            
            # Update access time
            self.index.update_times(path, atime=time.time())
            
            result = bytes(content[offset:offset+size])
            bytes_read = len(result)
            
            # Log successful read
            self._log_access(path, "read", bytes_read, start_time, success, error_code)
    ...
    def write(self, path, data, offset, fh):
        ...
        with self._lock:
            ...
            self._log_access(path, "write", bytes_written, start_time, success, error_code)
    ...
    def _flush_file(self, path: str):
        """Flush file to scattered storage with transaction support."""
        ...
        # Begin transaction
        tx_id = f"flush_{path}_{time.time()}"
        tx = self.tx_manager.begin_transaction(tx_id)
        
        try:
            # Scatter and store with transaction
            ref_id = self.storage.store(entry.file_id, bytes(content), tx_id)
            tx.add_operation('store_scatter', ref_id=ref_id, file_id=entry.file_id.hex())
            
            # Update entry
            old_ref = entry.scattered_ref
            entry.scattered_ref = ref_id
            tx.add_operation('update_entry', path=path, old_ref=old_ref, new_ref=ref_id)
            
            # Mark clean
            self.cache.mark_clean(path)
            tx.add_operation('mark_clean', path=path)
            ...
            try:
                ...
                print(f"Warning: Flush rollback failed: {rollback_error}")
            raise e
    
    def unlink(self, path):
        """Delete file. Thread-safe with transaction support."""
        with self._lock:
            path = self._get_full_path(path)
            
            # Begin transaction
            tx_id = f"unlink_{path}_{time.time()}"
            tx = self.tx_manager.begin_transaction(tx_id)
            ...
            try:
                ...
                # Backup for rollback
                tx.add_backup(f'index_entry_{path}', entry)
                tx.add_backup(f'cache_content_{path}', self.cache.get(path))
                
                # Delete scattered data with transaction
                if entry.scattered_ref:
                    self.storage.delete(entry.scattered_ref, tx_id)
                    tx.add_operation('delete_scatter', ref_id=entry.scattered_ref)
                
                # Remove from cache
                self.cache.remove(path)
                tx.add_operation('remove_cache', path=path)
                
                # Remove from index
                self.index.delete(path)
                tx.add_operation('delete_index', path=path)
                ...
                try:
                    ...
                    print(f"Warning: Unlink rollback failed: {rollback_error}")
                raise e
    ...
    def chmod(self, path, mode):
        """Change permissions. Thread-safe."""
        with self._lock:
            path = self._get_full_path(path)
            entry = self.index.get(path)
    ...
    def statfs(self, path):
        """Get filesystem statistics. Thread-safe."""
        with self._lock:
    ...
    def lock_file(self, path: str, passphrase: str) -> bool:
        """Lock a file with passphrase (accessed via ioctl or xattr). Thread-safe."""
        with self._lock:
            path = self._get_full_path(path)
    ...
    def unlock_file(self, path: str, passphrase: str) -> bool:
        """Unlock a locked file. Thread-safe."""
        with self._lock:
            path = self._get_full_path(path)

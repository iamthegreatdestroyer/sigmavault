...
def test_parallel_io():
    ...
    from sigmavault.core.parallel_io import ParallelIOManager, ParallelIOConfig
    
    config = ParallelIOConfig(max_concurrent=8)
    manager = ParallelIOManager(config)
    ...
    print(f"  Avg Read Time: {stats.read_time_ms:.2f}ms")
    print(f"  Avg Write Time: {stats.write_time_ms:.2f}ms")

...
class MetricsCollector:
    ...
    def get_metrics(self, name: Optional[str] = None) -> Dict[str, Metric]:
        """
        Get metrics by name (if provided) or all metrics.
        ...
        with self.lock:
            if name:
                ...
                if name in self.counters:
                    metric_type, help_text = self.metric_metadata.get(name, (MetricType.COUNTER, ""))
                    return {name: Metric(
                        name=name,
                    ...
                    metric_type, help_text = self.metric_metadata.get(name, (MetricType.GAUGE, ""))
                    return {name: Metric(
                        name=name,
                        metric_type=metric_type,
                ...
                for counter_name, counter_value in self.counters.items():
                    metric_type, help_text = self.metric_metadata.get(counter_name, (MetricType.COUNTER, ""))
                ...
                for gauge_name, gauge_value in self.gauges.items():
                    metric_type, help_text = self.metric_metadata.get(gauge_name, (MetricType.GAUGE, ""))

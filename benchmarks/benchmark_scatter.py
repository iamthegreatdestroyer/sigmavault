...
def run_scatter_benchmarks() -> Dict[str, BenchmarkResult]:
    ...
    try:
        # Create engine with 100MB medium size
        engine = DimensionalScatterEngine(key_state, medium_size=100 * 1024 * 1024)
        
        file_id_1kb = b"bench_1kb_file"
        file_id_1mb = b"bench_1mb_file"
        
        def scatter_1kb():
            return engine.scatter(file_id_1kb, data_1kb)
        
        def scatter_1mb():
            return engine.scatter(file_id_1mb, data_1mb)
        ...
        scattered_1kb = engine.scatter(file_id_1kb, data_1kb)
        scattered_1mb = engine.scatter(file_id_1mb, data_1mb)
        
        def gather_1kb():
            return engine.gather(scattered_1kb)
        
        def gather_1mb():
            return engine.gather(scattered_1mb)

...
class TestIntegration:
    ...
    def test_synthetic_data_with_anomaly_detector(self, tmp_path):
        ...
        # Use dates relative to now so get_recent_events finds them
        now = datetime.now()
        training_start = now - timedelta(days=30)
        
        # Generate normal data for training (last 30 days)
        ...
        # Generate anomalous data (during training period)
        anomaly_start = now - timedelta(days=15)
        anomaly_events = generator.generate_anomaly(
            pattern=PatternType.ANOMALY_EXFILTRATION,
            vault_id="test-vault",
            start_time=anomaly_start.replace(hour=3, minute=0)
        )
        ...
        # Detect on both normal and anomaly (returns tuple: is_anomaly, score, alert_level)
        _, normal_score, _ = detector.detect(normal_events[-50:])
        _, anomaly_score, _ = detector.detect(anomaly_events[:50])
        
        # Anomaly should have LOWER score (more negative = more anomalous in Isolation Forest)
        # This is counter-intuitive: lower scores indicate anomalies

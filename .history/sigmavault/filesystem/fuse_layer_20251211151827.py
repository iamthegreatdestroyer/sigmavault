...
class TransactionManager:
    ...
    def _execute_rollback(self, operation: Dict[str, Any], tx: TransactionState):
        ...
        if op_type == 'create_file':
            # Remove created file
            path = operation['path']
            if tx.get_backup(f'index_backup_{path}'):
                # Restore index entry
                pass  # Will be handled by index rollback
                
        elif op_type == 'delete_file':
            # Restore deleted file
            path = operation['path']
            backup_data = tx.get_backup(f'file_backup_{path}')
            if backup_data:
                # Restore file data
                pass  # Will be handled by storage rollback
            ...
            if backup_data:
                ...
                pass  # Will be handled by cache rollback

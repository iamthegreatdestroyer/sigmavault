...
class TestDetectionCycle:
    ...
    def test_detection_without_trained_model(self, ml_bridge, temp_vault):
        ...
        generator = SyntheticDataGenerator(temp_vault)
        events = generator.generate_sequence(
...
class TestStressScenarios:
    ...
    def test_concurrent_access_logging(self, ml_bridge):
        ...
        threads = []
        for i in range(10):
            t = threading.Thread(target=log_events, args=(i, 50))
            threads.append(t)
            t.start()
        ...
        # Verify events were logged (buffer is capped at 1000)
        assert len(ml_bridge.event_buffer) <= 1000

...
# Import from drivers package
import sys
sys.path.insert(0, str(Path(__file__).parent.parent))

from drivers.storage.base import (
    StorageBackend,
    StorageCapabilities,
    StorageStats,
    StorageError,
...
class TestFileStorageBackend:
    ...
    def file_backend(self, temp_file):
        ...
        backend = FileStorageBackend(temp_file, size=1024 * 1024)  # 1 MB
        yield backend
        backend.close()
    ...
    def test_persistence_across_reopen(self, temp_file):
        ...
        # Write data
        backend1 = FileStorageBackend(temp_file, size=1024)
